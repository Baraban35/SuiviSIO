<?php
  class Mysql
  {
    ...
    public function sauveTables(){
	   ...
	   $fic = gzopen($nomFic,'w');
	   $ressTables = mysql_query('show tables from '.$this->bd,  $this->con);
	   while ($lesTables = mysql_fetch_row($ressTables)) {
	      $uneTable = $lesTables[0];
	      if ($res = mysql_query('show create table '.$uneTable, $this->con)) {
	         $tb = mysql_fetch_row($res);
	         gzwrite($fic, $tb[1].";\n");
	         $contenu = mysql_query('select * from '.$uneTable, $this->con);
